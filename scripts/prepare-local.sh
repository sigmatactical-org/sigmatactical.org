#!/usr/bin/env bash
# Link or clone sigma-theme, build TypeScript, and patch the git dependency for local/CI builds.
set -euo pipefail
...
if [[ -n "$THEME_HELPER" ]]; then
  ...
  if [[ -d ../theme/ts ]]; then
    ...
    git clone --depth 1 https://github.com/sigmatactical-org/theme.git theme
  fi
  mkdir -p .cargo
